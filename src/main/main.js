...
function updateThemeMenu() {
  ...
  const themesMenu = menu.items.find(item => item.label === 'Themes');
  if (!themesMenu || !themesMenu.submenu) return;

  themesMenu.submenu.items.forEach(item => {
    if (item.type === 'radio') {
      const themeMap = {
        'GitHub': 'github',
      ...
      item.checked = themeMap[item.label] === currentTheme;
    }
  });
}
